...
int main(int argc, char *argv[])
{
	...
	// read video source
	// from video file
	std::cout << "video path:" << oCfg.getInVdoPth() <<"    type:" << oCfg.getInVdoTyp() << std::endl;
	if (0 == oCfg.getInVdoTyp()) 
	{
		//td::cout<<"--------------------------------------"<<std::endl;
		//std::cout<<oCfg.getInVdoPth()<<std::endl;
	...
	// from IP camera
		...
		//oVdoCap.open(oCfg.getInVdoPth()); // open a video file
		oVdoCap.open(0); // open cam
	// from updating image
	else if (3 == oCfg.getInVdoTyp())
		oImgFrm = cv::imread(oCfg.getInFrmPth(), IMREAD_COLOR);
	...
	{
		oFrmSz.width = (int)oVdoCap.get(3);
		oFrmSz.height = (int)oVdoCap.get(4);
	}
	else
	{
	...
	if (oCfg.getSelRoiFlg())
	{
		...
		oImgRoi = cv::imread(oCfg.getInRoiPth(), IMREAD_GRAYSCALE);
		...
		if (0 == oCfg.getInVdoTyp())
		{
			...
			//edited by us
			//cout<<"---------------------------------------------------";
			//cout<<"Video path:"<<oCfg.getInVdoPth();
		}
	...
	{
		if (0 == oCfg.getInVdoTyp())
			oVdoWrt = cv::VideoWriter(oCfg.getOutVdoPth(), VideoWriter::fourcc('M', 'P', '4', '2'), (double)oVdoCap.get(5), oFrmSz);
		else
			oVdoWrt = cv::VideoWriter(oCfg.getOutVdoPth(), VideoWriter::fourcc('M', 'P', '4', '2'), oCfg.getFrmRt(), oFrmSz);
	}
